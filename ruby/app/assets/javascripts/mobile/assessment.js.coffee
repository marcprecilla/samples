...
$ ->
  ...
  $('.single-select-question').each ->
    ...
    $(this).find('.btn-group .btn').click  (event) ->
      event.preventDefault()
      setTimeout(->
  ...
  if $('body#assessment.show').length > 0
  ...
  $('body#assessment.show .btn.flex-next').click (e) ->
    e.preventDefault()
    e.stopPropagation()
    input_valid = 1
  ...
  $('body#assessment.show .core-skills li').each ->
  ...
  if $('body#assessment.index').length > 0
